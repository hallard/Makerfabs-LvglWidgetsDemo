...
void touch_init(int max_x, int max_y)
{
...
#elif defined(TOUCH_GT911)
  // Begin init wire, and Reset the device also, no need to be done here
  ...
  returnSize = Wire.requestFrom(GT911_ADDR1, (uint8_t)1); //returns rxLength - if 0 we have a problem
  if (returnSize == 0) {
    SerialDebug.println("Setting address to ADDR2");
